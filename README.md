...
This repository is built around two ideas:

- serial cleaning should be easy for newcomers to install and use,
- parallel cleaning should become the star of the show when MPI resources are available.

In practice, that means the repo helps a user:

- clean spike-like outliers,
- build a spline-style dense signal representation,
- generate a short human-readable summary of where each signal looks steady versus where it starts changing again,
- scale from a simple serial run to replicated or partitioned MPI cleaning when resources justify it,
- and only then, when needed, run separate benchmark studies.
...
- New user: go to `Quick start`.
- Need MPI guidance: go to `Parallel Cleaning`.
- Need cache or larger-data guidance: go to `Running on GB-scale data`.
- Need benchmark tooling: go to `When to use benchmark scripts`.
- Need contributor/module context: go to `src/ncdt_cleaner/README.md`.
...
- `reports/` - optional writeup and figure-generation assets
- `c_openmp/` - focused C/OpenMP comparison kernel
- `tests/` - pytest coverage plus a local smoke-test helper

## Quick start

From the repo root, create or activate the environment for serial use:
...
If you want parallel cleaning support as well, install the MPI extra:
...
```bash
ncdt-cleaner workflow examples/tiny_sensor_example.csv \
  --characterize
```
...
Supported raw input formats currently include `.csv`, `.xlsx`, `.lvm`,
`.json`, `.ndjson`, `.h5`, and `.hdf5`.

## Parallel Cleaning

Parallel cleaning is a first-class feature of the repo, not just a benchmark
topic.

- Use `serial` for your first pass and for smaller datasets.
- Use `replicated` when you have a few MPI ranks and enough memory for each rank to hold the full dataset.
- Use `partitioned` when you have more MPI ranks or larger row counts and want the more scalable time-domain decomposition.

The default workflow execution behavior now comes from `configs/default_config.json`
under `execution`, while optional repeated timing studies stay under `benchmark`.
...
```bash
ncdt-cleaner workflow /path/to/noisy_input.csv \
...
1. run `workflow examples/tiny_sensor_example.csv --characterize`,
...
The default workflow posture is analysis-first. Repeated benchmark timing runs
are optional and are controlled either by `benchmark.enabled_by_default` in
`configs/default_config.json` or by explicit CLI overrides:

```bash
ncdt-cleaner workflow /path/to/noisy_input.csv --characterize
ncdt-cleaner workflow /path/to/noisy_input.csv --characterize --benchmark
ncdt-cleaner workflow /path/to/noisy_input.csv --characterize --skip-benchmark
```
...
- `workflow/benchmark/benchmark_summary.csv`: optional strong-scaling summary table when benchmarking is enabled.
...
- `*_signal_overlay.png`: per-sensor overlays of raw data, cleaned signal, and spline/interpolated curve.
...
5. `analysis/<latest_session>/outputs/clean_serial/sensors/*_signal_overlay.png`
...
- and signal overlays showing raw data, cleaned signal, and the spline/interpolated curve.
...
```bash
ncdt-cleaner workflow /path/to/large_input.csv \
  --characterize
```
...
If you explicitly want the performance study as part of the workflow, add:
...
Those scripts are for performance studies and writeup figures, not the first pass
...
- or you are generating larger performance-study results.
...
If you are preparing a writeup or audit trail, keep the benchmark and
summary-producing sessions. If you are only exploring thresholds or testing
quick ideas, it is reasonable to prune older sessions regularly.

## Advanced writeup workflow

The optional writeup assets in `reports/` come from the same code paths described above.
...
2. Run the stronger MPI studies from `benchmarks/` when you need scaling figures for a review or writeup:
...
3. Regenerate the combined summary figures:
...
4. Build the LaTeX writeup in `reports/`.
...
7. **Benchmark** serial vs MPI strategies when you need performance evidence.
8. Optionally use the generated JSON/CSV/plots in the writeup assets under `reports/`.

The recommended entrypoint is now:

```bash
ncdt-cleaner workflow /path/to/Test00.xlsx
```

If you do not pass `--modes`, the workflow now takes its default mode list from
`configs/default_config.json` under `execution.default_modes`.

That same `workflow` entrypoint also supports LabVIEW `.lvm` exports when the
file contains one of the standard tabular segment layouts used in the sample
data deposits.
...
- optionally benchmark the requested process counts,
- write a consolidated `workflow_report.json`,
- emit analysis-friendly files such as `benchmark_summary.csv`, timing-breakdown summaries, and benchmark plots when benchmarking is enabled.

If you already have a cache, reuse it directly:

```bash
ncdt-cleaner workflow --cache-dir analysis/<session>/cache
```
...
This distinction matters in technical reviews because the spline step should not be
...
The optional scientific-paper-style draft is in:
