...
This module contains the two parallel execution modes supported by the repo:
...
def run_replicated_mode(
    ...
    """Run the replicated-data MPI strategy."""
                ...
                characterization_rows.append(
                    {
                        ...
                        "signal_overlay_plot": (
                            plot_signal_overlay(
                                ...
                                payload["cleaned"],
                                payload["characterization"].get("dense_time") if "characterization" in payload else None,
                                payload["characterization"].get("dense_values") if "characterization" in payload else None,
                                Path(sensors_dir) / f"{sensor_key}_signal_overlay.png",
                                title=f"{sensor}: raw, cleaned, and spline overlay",
                                steady_segments=payload["steady_state"].get("steady_segments"),
                            )
                            if do_plots and "steady_state" in payload
                            else None
                        ),
                ...
                characterization_rows.append(
                    {
                        ...
                        "signal_overlay_plot": (
                            plot_signal_overlay(
                                ...
                                dataset.sensors[sensor],
                                cleaned,
                                characterization.get("dense_time"),
                                characterization.get("dense_values"),
                                Path(sensors_dir) / f"{sensor_key}_signal_overlay.png",
                                title=f"{sensor}: raw, cleaned, and spline overlay",
