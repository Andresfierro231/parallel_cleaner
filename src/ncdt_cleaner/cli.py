...
inspection, cache creation, cleaning, and optional performance studies.
'''
...
from .plotting import plot_signal_overlay
...
MODE_HELP = (
    "serial = easiest first pass and smallest dependency surface; "
    "replicated = good for a few MPI ranks when each rank can hold the full dataset; "
    "partitioned = better for larger datasets or higher rank counts where time-domain decomposition scales better."
)
...
def _add_plot_toggle_arg(parser: argparse.ArgumentParser) -> None:
    """Add the user-facing plot generation toggle to a parser."""
    parser.add_argument(
        "--skip-plots",
        action="store_true",
        help="Do not generate per-sensor plot images for this run.",
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--mode",
        choices=["serial", "replicated", "partitioned"],
        default="serial",
        help=f"Cleaning mode to run. {MODE_HELP}",
    )
    p.add_argument("--characterize", action="store_true")
    _add_steady_state_override_arg(p)
    _add_plot_toggle_arg(p)
    ...
    p.add_argument(
        ...
        choices=["serial", "replicated", "partitioned"],
        default=None,
        help=f"Cleaning modes to execute. Defaults come from execution.default_modes in the config. {MODE_HELP}",
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="MPI rank count for one-off replicated/partitioned analysis runs. If omitted, the config or largest benchmark process count greater than 1 is used.",
    )
    p.add_argument(
        "--mpi-launcher",
        default="mpirun",
        help="MPI launcher to use for replicated/partitioned analysis runs. Defaults to mpirun unless overridden by config execution.mpi_launcher.",
    )
    p.add_argument("--characterize", action="store_true")
    ...
    benchmark_group.add_argument(
        ...
        dest="benchmark",
        action="store_true",
        help="Run repeated benchmark timing studies after the analysis runs.",
    )
    benchmark_group.add_argument(
        "--skip-benchmark",
        dest="benchmark",
        action="store_false",
        help="Skip benchmark timing studies even if the config enables them.",
    )
    p.set_defaults(benchmark=None)
    _add_steady_state_override_arg(p)
    _add_plot_toggle_arg(p)
                ...
                characterization_rows.append(
                    {
                        ...
                        "signal_overlay_plot": (
                            plot_signal_overlay(
                                ...
                                result.cleaned,
                                char.get("dense_time"),
                                char.get("dense_values"),
                                sensors_dir / f"{normalize_header(sensor)}_signal_overlay.png",
                                title=f"{sensor}: raw, cleaned, and spline overlay",
        ...
        rows.append(
            {
                ...
                "signal_overlay_plot": (
                    plot_signal_overlay(
                        ...
                        char.get("dense_time"),
                        char.get("dense_values"),
                        out_dir / f"{normalize_header(sensor)}_signal_overlay.png",
                        title=f"{sensor}: raw data and spline overlay",
                        steady_segments=behavior.get("steady_segments"),
                    )
                    if not args.skip_plots
                    else None
