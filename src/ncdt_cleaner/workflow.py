'''
File description:
High-level workflow orchestration for the end-to-end analysis pipeline.

This module is the glue that reduces the repo's previous manual steps into one
command that can inspect data, build cache files, run analysis modes, and, when
requested, emit benchmark artifacts for performance studies.
'''
...
def run_workflow(args, cfg: dict, session: dict) -> dict[str, Any]:
    ...
    benchmark_enabled = (
        ...
        else bool(benchmark_cfg.get("enabled_by_default", False))
    )
    selected_modes = _selected_modes(args.modes or execution_cfg.get("default_modes"))
    ...
    analysis_nproc = _analysis_nproc(process_counts, args.analysis_nproc or execution_cfg.get("analysis_nproc"))
    steady_window_seconds = args.steady_window_seconds
    mpi_launcher = args.mpi_launcher or execution_cfg.get("mpi_launcher", "mpirun")
    ...
    if not args.skip_clean_runs:
        for mode in selected_modes:
            ...
            if mode != "serial":
                ...
                command = mpi_wrapped_command(command, nproc=nproc, mpi_launcher=mpi_launcher)
    ...
    if benchmark_enabled:
        ...
        # timing-study artifacts reflect repeated measurements, not setup work.
    ...
    report = {
        ...
        "paper_artifacts": reference_artifacts,
    }
